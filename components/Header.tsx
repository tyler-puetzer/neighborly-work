...
export function Header(){
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500`}>
    <div className={`mx-auto flex max-w-7xl items-center justify-between rounded-none border-0 px-4 py-3 transition-all duration-500 ${scrolled?'border-slate-200/80 bg-white/88 shadow-soft backdrop-blur-xl':'border-transparent bg-white/30 backdrop-blur-md'}`}>
      ...
      <nav className="hidden items-center gap-7 lg:flex">{nav.map(([label,href])=>href.startsWith('http')?<a key={label} href={href} target="_blank" rel="noopener noreferrer" className="text-[13px] font-semibold text-slate-700 transition hover:text-[#0894F0]">{label}</a>:<Link key={label} href={href} className="text-[13px] font-semibold text-slate-700 transition hover:text-[#0894F0]">{label}</Link>)}</nav>
    ...
    <AnimatePresence>{open&&<motion.div initial={{opacity:0,y:-12}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-12}} className="mx-3 mt-2 rounded-3xl border border-slate-200 bg-white p-4 shadow-soft lg:hidden">{nav.map(([label,href],i)=><motion.div key={label} initial={{opacity:0,x:-8}} animate={{opacity:1,x:0}} transition={{delay:i*.04}}>{href.startsWith('http')?<a href={href} target="_blank" rel="noopener noreferrer" onClick={()=>setOpen(false)} className="block rounded-2xl px-4 py-4 text-base font-semibold text-slate-800 hover:bg-slate-50">{label}</a>:<Link href={href} onClick={()=>setOpen(false)} className="block rounded-2xl px-4 py-4 text-base font-semibold text-slate-800 hover:bg-slate-50">{label}</Link>}</motion.div>)}<Link href="/contact" onClick={()=>setOpen(false)} className="btn-primary mt-2 w-full">Request Service</Link></motion.div>}</AnimatePresence>
  </header>
}
