...
export function Header(){
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled?'py-3':'py-5'}`}>
    <div className={`mx-auto flex max-w-7xl items-center justify-between rounded-full border px-4 py-3 transition-all duration-500 ${scrolled?'border-slate-200/80 bg-white/88 shadow-soft backdrop-blur-xl':'border-transparent bg-white/30 backdrop-blur-md'}`}>
      <Link href="/" className="relative z-10 flex items-center" onClick={()=>setOpen(false)} aria-label="Neighborly Work home">
        <Image src="/brand/neighborly-works-logo-horizontal-on-light.svg" alt="Neighborly Work" width={178} height={34} priority className="h-7 w-auto"/>
      </Link>
      ...
      <div className="hidden lg:block"><Link href="/contact" className="btn-primary px-5 py-3 text-sm">Get a Quote</Link></div>
